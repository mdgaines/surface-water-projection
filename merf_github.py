...
class MERF(object):
    ...
    * bi is the random effect coefficients. They are different per cluster i but are assumed to be drawn from the same distribution ~N(0, Sigma_b) where Sigma_b is learned from the data.
    ...
    def predict(self, X: np.ndarray, Z: np.ndarray, clusters: pd.Series):
        ...
        y_hat = trained_fe_model.predict(X)
    ...
    def fit(
        ...
        sigma2_hat_history.append(sigma2_hat)
        D_hat_history.append(D_hat)

        early_stop_flag = False

        while iteration < max_iterations and not early_stop_flag:
            iteration += 1
            logger.debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            ...
            fe_model.fit(X, y_star)
            f_hat = fe_model.predict(X)
            ...
            b_hat_history.append(b_hat_df.copy())
            sigma2_hat_history.append(sigma2_hat)
            D_hat_history.append(D_hat)
            ...
            if gll_early_stop_threshold is not None and len(gll_history) > 1:
                curr_threshold = np.abs((gll - gll_history[-2]) / gll_history[-2])
            ...
            if X_val is not None:
                yhat_val = predict(X_val, Z_val, clusters_val)
